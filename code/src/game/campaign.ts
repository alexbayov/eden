...
/**
 * Explicit voluntary exit from an active encounter. Modelled exactly like a defeat for
 * progression purposes — the encounter is `failed`, the campaign moves to `return` and no
 * reward becomes claimable — but the hero is evacuated instead of being knocked out, so the
 * surviving HP/statuses are carried by the caller. This is the only safe way out of a combat
 * soft lock (no ammo, no valid attack).
 */
export const retreatFromMission = (state: CampaignState): CampaignState => missionDefeat(state)
