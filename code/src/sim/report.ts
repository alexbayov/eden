...
export function renderMarkdown(report: SimulationReport): string {
  ...
  /* Kept accurate as the game grows: each of these lines was true when written and two of them stopped being true, which
     is exactly how a report starts lying about itself. */
  lines.push('- Навыки и перки: их нет в коде (`W4-03`/`W4-04`, решение D-02 — post-MVP), поэтому уровень не влияет ни на одну боевую формулу. Уровни и XP при этом реализованы и растут.')
  lines.push(
  ...
  lines.push(
    report.restock
      ...
      : '- Восстановление между encounter: не выполняется вообще (запуск без `--restock`), поэтому это пессимистичная нижняя граница — герой не восполняет патроны, не лечится и не ремонтируется.',
  )
  lines.push('')
  return lines.join('\n')
}
