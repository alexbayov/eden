...
В `code/` реализованы одна зона, три data-driven encounter, runtime map switching через validated arena catalog, campaign progression, прогрессия персонажа L1–L6 со штрафом смерти, **schema-v5** local save (ключ `eden.save.v5`, миграция v4→v5) и responsive/a11y UX layer. Числа тестов в этом абзаце устарели (см. предупреждение выше; актуальные — [`docs/24`](docs/24-test-matrix-and-release-gates.md) §1: **23 файла / 260 vitest** и **7 файлов / 82 Playwright** на 24 августа 2026). Исторические числа: **19 vitest files / 147 automated tests** и **7 Playwright files / 63 browser tests**. Статический бюджет бандла: initial JS 36.2 kB gzip (лимит 150), combat lazy JS 349.7 kB gzip (порог 1200) — фактический вывод `npm run analyze:budget` на 23 августа 2026; DOM- и E2E-инфраструктура на бандл не влияет (dev-only).
