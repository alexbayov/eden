...
**Фактическое состояние набора на 24 августа 2026 (после W4-01/W4-02/W4-05, в рабочем дереве):** `npm test` собирает **23 файла / 260 тестов** — 21 файл / 234 теста в project `node` и 2 файла / 26 тестов в project `dom`. **Набор красный:** 259 зелёных, **1 падает** — `combat-shell.dom.test.tsx` («уровень и прогресс на экране награды»: тест ждёт блок нераспределённых очков, шелл его не выводит). `npm run lint` и `npm run typecheck` зелёные (**проверено**). `npm run test:e2e` — **7 файлов / 82 теста** в Chromium (`viewport-geometry` 35, `save-reload` 20, `campaign-failure` 9, `input-gating` 7, `campaign-happy-path` 5, `smoke` 4, `corrupt-save` 2), состав получен через `playwright test --list`; прогон набора в этой правке не выполнялся. Владелец этих чисел — [`24-test-matrix-and-release-gates.md`](24-test-matrix-and-release-gates.md) §1–2.

**Пакет `W4` реализован, но как гейт не закрыт.** `W4-01` (кривая L1–L6 как контент), `W4-02` (единый XP-штраф смерти) и `W4-05` (save v5 + миграция v4→v5) есть в коде; verify/critic-проход по ним **не выполнен**, и `npm run verify` не может быть зелёным, пока красный `npm test`. Числа и формула — [`05-character-and-progression.md`](05-character-and-progression.md) §5.3.0. `W4-03`/`W4-04` не начаты.
