...
**Разрыв до полного MVP — пересчитан 27 августа 2026, после решений D-01, D-02, D-03, D-05 и D-06.**

| Было в разрыве | Состояние на 27 августа 2026 |
|---|---|
| 1 зона из 4 | **2 зоны из 2** — D-03 зафиксировал объём как 2 зоны по 3 encounter; `W7-01` закрыт целиком |
| навыки/SPECIAL/перки | **вне MVP** по решению D-02 (post-MVP), а не «не сделано» |
| реестр ассетов и fallback | **сделан** (`W8-02`); релиз на placeholder — решение D-06 |
| платформенная граница | **сделана** (`W10-02`): no-op по умолчанию, нулевой сетевой трафик проверен в браузере |

**Что действительно осталось в разрыве:** финальная миссия и нарратив (`W7-03`/`W7-04` — нет implementation spec), production-арт и аудио (`W8-03`…`W8-05`, `W9-01`/`W9-02` — вне объёма по D-06), интеграция с Яндекс.Играми (`W10-01`/`W10-03` — нет локальной документации SDK), решение **D-04** по Phaser 3/4 и ручная QA на устройствах и со скринридером. Всё перечисленное заблокировано **внешними входами или решениями владельца**, а не объёмом разработки. **Реализовано 24 августа 2026 и проверено набором, ожидает только финального critic-прохода:** прогрессия уровней L1–L6 (`W4-01`), единый XP-штраф смерти (`W4-02`), сохранение schema v5 с миграцией v4→v5 (`W4-05`). `npm test` — 23 файла / **263 теста, все зелёные**, `npm run verify` проходит (exit 0); прежняя запись «красный, 259 из 260, падает `combat-shell.dom.test.tsx`» **неверна и снята**. Полностью закрытым пакет всё же не называется: critic-прохода не было, а решение D-01 владельцем не принято, поэтому вариант штрафа остаётся решением реализации. **Не реализовано и остаётся в разрыве:** навыки, SPECIAL и перки (`W4-03`/`W4-04`, решение D-02) и лут-штраф с рюкзака (`W5-05`) — поражение не отнимает ни ресурсы, ни предметы. Порядок закрытия остатка — [`21-delivery-plan-and-backlog.md`](21-delivery-plan-and-backlog.md).
