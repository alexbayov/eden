...
**Разрыв до полного MVP.** Реализованный слайс — 1 зона из 4, 3 архетипа врага из 6, 2 ресурса из 8, 1 рецепт, 1 улучшение базы, без навыков/SPECIAL/перков, без туториала, без финальной миссии, без ассетов, аудио и платформенных интеграций. **Реализовано 24 августа 2026, но как гейт не закрыто:** прогрессия уровней L1–L6 (`W4-01`), единый XP-штраф смерти (`W4-02`), сохранение schema v5 с миграцией v4→v5 (`W4-05`). Формулировка «закрыто» была бы неверной: verify/critic-прохода по `W4` нет, `npm test` в рабочем дереве красный (259 из 260 — падает `combat-shell.dom.test.tsx`), решение D-01 владельцем не принято. Порядок закрытия остатка — [`21-delivery-plan-and-backlog.md`](21-delivery-plan-and-backlog.md).
