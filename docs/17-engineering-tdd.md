...
**Фактический набор на 24 августа 2026 (рабочее дерево, после `W4-01`/`W4-02`/`W4-05`).** `npm test` — **23 файла / 263 теста, все зелёные** (~3.5 с): 21 файл / 234 теста в project `node`, 2 файла / **29** тестов в project `dom` (jsdom). `npm run verify` **проходит целиком, exit 0** (**проверено** прогоном). Прежняя запись «набор красный: 259 зелёных, 1 падает `combat-shell.dom.test.tsx`» и разбивка «2 файла / 26 тестов» **неверны и сняты**: падающего теста нет ни в рабочем дереве, ни на коммите `129e2be` (там 260 зелёных). `npm run test:e2e` — **7 файлов / 82 теста** в Chromium (состав получен через `playwright test --list`; прогон в этой правке не выполнялся). Числами владеет [`24-test-matrix-and-release-gates.md`](24-test-matrix-and-release-gates.md) §1–2 — при расхождении верен тот документ.
...
- `game/progression.ts` — единственная реализация кривой уровней, начисления XP и штрафа смерти (`W4-01`/`W4-02`); кривая живёт в контенте (`public/config/progression.json`), и ни один экран не пересчитывает уровень или штраф самостоятельно. Docblock модуля описывает **действующую** кривую `0 / 40 / 90 / 155 / 235 / 330` — прежняя запись о расхождении комментария с константой неверна и снята. `W4-01`/`W4-02`/`W4-05` **реализованы и проверены зелёным `verify`**, ожидают только финального critic-прохода; `W4-03`/`W4-04` (SPECIAL, навыки, перки) **не реализованы**, лут-штраф `W5-05` — тоже. Числа и формула — [`05-character-and-progression.md`](05-character-and-progression.md) §5.3.0.
...
Final gate: `npm run verify` (`lint`, `typecheck`, `test`, `build`, `analyze:budget`) — на 24 августа 2026 **проходит целиком локально, exit 0** (23 файла / 263 теста; initial JS 38.9 kB gzip, ленивый бой 349.7 kB gzip). Прежняя запись «не проходит из-за одного падающего DOM-теста» неверна и снята. Browser E2E и измеренная геометрия — проверены локально в Chromium (`W1-01`…`W1-05`); CI job `e2e` на GitHub Actions последним прогоном был **красный** (`webServer` timeout, DEF-3); runtime performance profiling, cross-browser матрица и ручная mobile QA остаются непроверенными.
